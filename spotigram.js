...
async function login(u, p) {
    ...
    try {
        ...
        console.log('Something went wrong while logging in (incorrect username or password?), Try again!'.red)
        console.log('- If your account has 2fa/mfa, support for this will come in futre updates. Stay tuned!'.yellow)
        return tryLogin();
    }
...
async function get() { //main bio update function
    if(!config.accessToken || !config.refreshToken) {
        return console.log('You have not linked your spotify yet. Do it manually by going to http://localhost:8000/link in your browser.'.yellow)
    } else {
        setInterval(() => {
